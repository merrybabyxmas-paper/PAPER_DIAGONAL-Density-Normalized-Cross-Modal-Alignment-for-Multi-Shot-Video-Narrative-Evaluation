...
Adding to $X$:
\begin{equation}
X + \sigma_{12} \circ \calB_2 = (E_{11} - E_{21}) + (-E_{11} + E_{21} - E_{22}) = -E_{22}.
\end{equation}
Thus $E_{22} = -(X + \sigma_{12} \circ \calB_2)$.

\textbf{Step~3: Propagation.}
From $E_{22}$, character role swapping yields $E_{21} = \sigma_{12} \circ E_{22}$ and $E_{23} = \sigma_{23} \circ E_{22}$.
From $X = E_{11} - E_{21}$ and the now-known $E_{21}$: $E_{11} = X + E_{21}$.
Similarly, $E_{12} = \sigma_{12} \circ E_{11}$ and $E_{13} = \sigma_{13} \circ E_{11}$.
...
For any target column $k \in \{1, \ldots, N\}$, there exists a character role swapping $\sigma \in \Sigma_N$ such that:
\[
