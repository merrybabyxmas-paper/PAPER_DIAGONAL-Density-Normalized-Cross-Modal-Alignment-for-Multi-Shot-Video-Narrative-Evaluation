...
Under character role swapping $\sigma_{ij}$ and integer-coefficient linear combination, the two basis matrices $\calB_1$ (Relay) and $\calB_2$ (Split) span $\mathbb{M}_{2 \times 3}(\mathbb{Z})$.
\end{theorem}

\begin{proof}
We construct all six standard basis matrices $E_{ij}$ from $\calB_1$ and $\calB_2$.

\emph{Step~1: Column isolation.}\;
Define three character role swapping variants of $\calB_1$:
\begin{align}
v_1 &= \calB_1 = E_{12} - E_{21}, \notag \\
v_2 &= \sigma_{13} \circ \calB_1 = E_{12} - E_{23}, \notag \\
v_3 &= \sigma_{23} \circ \sigma_{12} \circ \calB_1 = E_{11} - E_{23}. \notag
\end{align}
Computing $X = v_3 - v_2 + v_1$:
\begin{flalign}
& X = (E_{11} - E_{23}) - (E_{12} - E_{23}) + (E_{12} - E_{21}) = E_{11} - E_{21}. \label{eq:supp_e11_e21} &&
\end{flalign}

\emph{Step~2: Dimension breaking via $\calB_2$.}\;
Apply character role swapping: $\sigma_{12} \circ \calB_2 = -E_{11} + E_{21} - E_{22}$.
Adding to $X$:
\begin{flalign}
& (E_{11} - E_{21}) + (-E_{11} + E_{21} - E_{22}) = -E_{22}. \label{eq:supp_e22} &&
\end{flalign}
Thus $E_{22} = -(X + \sigma_{12} \circ \calB_2)$.

\emph{Step~3: Propagation.}\;
From the now-known $E_{22}$, the remaining five elementary matrices follow:
\begin{equation}
\label{eq:supp_propagation}
\begin{cases}
...
In the main paper (\S3), Theorem~1 proved that the basis set $\calB = \{\calB_1, \calB_2\}$ spans the local narrative space $\mathbb{M}_{2\times 3}(\mathbb{Z})$ under the symmetric group of character role swappings $\Sigma_3$.
...
\emph{Linearity.} For any $c_1, c_2 \in \mathbb{Z}$ and $X, Y \in \mathbb{M}_{2\times 3}$:
\begin{equation}
...
\emph{Linearity.} For any $c_1, c_2 \in \mathbb{Z}$ and $X, Y \in \mathbb{M}_{2\times N}$:
\begin{equation}
...
\begin{lemma}
The set $\hat{\calB} = \{\hat{\calB}_1, \hat{\calB}_2\}$ spans $\mathbb{M}_{2\times N}(\mathbb{Z})$ under the character role swapping group $\Sigma_N$.
\end{lemma}
...
For any target column $k \in \{1, \ldots, N\}$, there exists a character role swapping $\sigma \in \Sigma_N$ such that $E_{i,k}^{2\times N} = \sigma\bigl(\hat{E}_{i,1}^{2\times N}\bigr)$.
...
\begin{equation}
\Delta S = \sum_{\tau=1}^{T-2} \sum_{m} c_{\tau,m}\;\, \mathcal{S}_\tau \circ \sigma_{\tau,m} \circ \hat{\calB}_m,
...
We compare the discriminative power of three candidate metrics: PTM ($\ell_0$-norm on prescribed cells), EPA (entity presence accuracy), and raw Frobenius distance $\|E\|_F$.
...
To verify that model rankings are not driven by a single easy/hard pattern, we compute per-pattern model rankings and measure concordance via Kendall's coefficient of concordance $W$.
