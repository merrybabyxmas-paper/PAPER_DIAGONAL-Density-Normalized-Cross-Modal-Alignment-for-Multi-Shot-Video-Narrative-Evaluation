...
In Theorem~3.1 (main text), we established that $\{\calB_1, \calB_2\}$ spans $\mathbb{Z}^{2 \times 3}$. To avoid redundancy with the main text's constructive proof, we provide the formal algebraic proof of why $\calB_1$ (Relay) alone is strictly insufficient to span the space, thereby necessitating the introduction of $\calB_2$ (Split).
...
\paragraph{Spatial Zero-Padding Operator ($\mathbb{Z}_N$).}
Appends $N-3$ zero-columns to accommodate arbitrary entity counts:
\begin{align}
\mathbb{Z}_N(X) = \bigl[\; X \;\;\; \mathbf{0}_{2\times(N-3)} \;\bigr], \quad X \in \mathbb{Z}^{2\times 3}.
\end{align}
\emph{Linearity.}\; $\mathbb{Z}_N(c_1 X + c_2 Y) = c_1\mathbb{Z}_N(X) + c_2\mathbb{Z}_N(Y)$.

\noindent\emph{Video interpretation.}\; In a generated video, the basis patterns $\calB_1$ and $\calB_2$ are defined over 3 entity slots. When a narrative involves $N > 3$ entities (e.g., 5 characters), $\mathbb{Z}_N$ embeds the 3-entity transition into the wider $N$-entity space by filling the additional entity columns with zeros---meaning those extra characters are unaffected by this particular local transition.

We define the padded bases $\hat{\calB}_1 = \mathbb{Z}_N(\calB_1)$ and $\hat{\calB}_2 = \mathbb{Z}_N(\calB_2)$.
...
\begin{align}
\mathcal{S}_\tau(X) = \begin{bmatrix} \mathbf{0}_{(\tau-1)\times N} \\ X \\ \mathbf{0}_{(T-\tau-2)\times N} \end{bmatrix}, \quad X \in \mathbb{Z}^{2\times N}.
\end{align}
\emph{Linearity.}\; $\mathcal{S}_\tau(c_1 X + c_2 Y) = c_1\mathcal{S}_\tau(X) + c_2\mathcal{S}_\tau(Y)$.

\noindent\emph{Video interpretation.}\; A long video with $T$ shots contains $T-1$ consecutive shot-to-shot transitions. Each basis pattern covers only a 2-row (i.e., 2-transition) local window. $\mathcal{S}_\tau$ places this local transition at temporal position $\tau$ within the full $(T{-}1)$-row timeline, padding the remaining rows with zeros---meaning shots outside this window are unaffected. Together, $\mathbb{Z}_N$ and $\mathcal{S}_\tau$ allow any narrative of arbitrary length and cast size to be decomposed into a sum of localized basis operations.
...
\begin{proof}
Any $\Delta S$ can be decomposed into $T-2$ overlapping local transitions $\Delta S^{(\tau)}$. By Theorem~3.1 (main text), each local block is spanned by the extended bases $\hat{\calB}$. Because both $\mathbb{Z}_N$ and $\mathcal{S}_\tau$ are linear homomorphisms, their composition perfectly preserves the spanning property across the globally expanded dimensions.
\end{proof}
...
\midrule
SD & SDXL+SEINE & T2I$\to$I2V$^\dagger$ & 30+50 & 512$\times$512 & 16 \\
Echo  & Wan2.1-1.3B & T2V (FM) & 50 & 832$\times$480 & 93 \\
...
Convergence   & 3 & 78.8 & 81.0 & 47.5 & 14.3 \\
...
(2)~The middle frame achieves the highest PTM for SD (.272) and the highest EPA overall, consistent with our design rationale that mid-shot frames maximize distance from transition artifacts at shot boundaries.
